...
class OrderProvider : CommandArgumentProvider<OrderArgument> {
    ...
    override suspend fun cast(context: CommandContext, argument: String?): OrderArgument {
        if (argument == null) throw ArgumentFeedback.Message("§c순서를 입력해주세요.")
        val order = argument.toIntOrNull() ?: throw ArgumentFeedback.Message("§c숫자만 입력할 수 있습니다.")
        if (order < 1) throw ArgumentFeedback.Message("§c양의 정수만 입력할 수 있습니다.")
